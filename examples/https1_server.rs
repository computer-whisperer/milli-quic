//! HTTPS/1.1 server example — HTTP/1.1 over TLS 1.3.
//!
//! Binds a TCP socket on `0.0.0.0:9443`, performs a TLS 1.3 handshake
//! (self-signed Ed25519 cert), then speaks HTTP/1.1 inside the tunnel.
//!
//! Usage:
//!   cargo run --example https1_server --features "http1,tcp-tls,rustcrypto-chacha"
//!
...
fn main() {
    ...
    // -- Bind TCP socket --
    let listener = TcpListener::bind("0.0.0.0:9443").expect("failed to bind TCP on :9443");
    println!("[init] listening on 0.0.0.0:9443 (TCP+TLS, HTTPS/1.1)");
    ...
    let mut tls: TlsServer<Aes128GcmProvider, 40960> =
        TlsServer::new(Aes128GcmProvider, tls_config, secret, random);

    let mut http1 = Http1Server::<40960, 2048, 4096>::new();
    ...
    loop {
        ...
        while let Some(event) = tls.poll_event() {
            match event {
                ...
                TlsEvent::AppData => {
                    // Read decrypted data → feed to HTTP/1.1
        ...
        // 4. HTTP/1.1 output → encrypt via TLS
        if tls_active {
...
fn route_http1(http1: &mut Http1Server<40960, 2048, 4096>, stream_id: u64, path: &str) {
    match path {
        "/status/404" => {
